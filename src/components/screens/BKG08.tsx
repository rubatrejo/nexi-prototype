    ...
    <div style={{ width: "100%", height: "100%", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 2, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{
          ...
          {/* Title */}
          <p style={{ fontSize: "0.5rem", fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 4 }}>Your Booking Ticket</p>
          <div style={{ fontFamily: "var(--font-display)", fontSize: "1rem", fontWeight: 800, color: "#fff", letterSpacing: 1.5, marginBottom: 4 }}>BKG-2026-73951</div>
          <div style={{ fontSize: "0.625rem", color: "rgba(255,255,255,0.5)", marginBottom: 16 }}>Deluxe King · Feb 22-25 · 3 nights</div>
          ...
          <div style={{ width: 120, height: 120, background: "#fff", borderRadius: "var(--radius-sm)", margin: "0 auto 6px", display: "flex", alignItems: "center", justifyContent: "center", padding: 8 }}>
            <div style={{ width: "100%", height: "100%", display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gridTemplateRows: "repeat(8, 1fr)", gap: 1.5 }}>
              {Array.from({ length: 64 }).map((_, i) => (
                <div key={i} style={{ background: [0,1,2,3,6,7,8,15,16,23,24,31,32,39,40,47,48,55,56,57,58,59,62,63,10,12,18,20,26,28,34,36,42,44,50,52].includes(i) ? "#000" : "#fff", borderRadius: 0.5 }} />
              ))}
            </div>
          </div>
          <p style={{ fontSize: "0.5625rem", color: "rgba(255,255,255,0.4)", marginBottom: 16 }}>Scan with your phone to save</p>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn btn-ghost" onClick={() => navigate("ONB-02")} style={{ flex: 1, fontSize: "0.6875rem", color: "#fff", borderColor: "rgba(255,255,255,0.25)", background: "rgba(255,255,255,0.06)" }}>Return Home</button>
